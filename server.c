...
void errorEncode(char ep[4], short int errMessage){
...
void errorDecode(int r, char code[], char errMessage[128]){
	short int tmp, en ;
	memcpy(&tmp, code+2, 2);
	en = ntohs(tmp); 
	switch(en){
...
void RWdecode(int r, char code[139],short int *opcode, char filename[128], char mode[9]){
...
void ACK(char code[4], short int *opcode, short int *blockno){
	short int tmp;
...
int main(){
	...
	if(checkcode(rd)==1 || checkcode(rd)==2){
		RWdecode(r, rd, &opcode, filename, mode);
	}else if(checkcode(rd)==5){
		//error
		char errMessage[128];
		errorDecode(r, rd, errMessage);
	...
	if(opcode == 1){
		...
		while((byteread = read(fd, buf, sizeof(buf)))!=0){
			...
			if(checkcode(ack)==5){
				//error
				char errMessage[128];
                		errorDecode(r, ack, errMessage);
 		                printf("!!! got err message : %s!!!\n", errMessage);
				return 1;
			}else if(checkcode(ack)==4){
				ACK(ack, &opcode2, &blockno2);
