...
void ACK(char ack[4], short blockno){	
	short int tmp;
	tmp = htons(4);
	memcpy(ack, &tmp, 2);
	tmp = htons(blockno);
...
int RWEncode(char bd[139], short int opcode, char filename[128], char mode[9]){
...
int main(){
	...
	char bd[139];
        int len = RWEncode(bd, 1, "R.txt", "octet");
	...
	if(checkcode(dp)==5){
		...
		printf("========data==============\n%s\n===============\n", data);
		int fd = open("Wr.txt", O_WRONLY);
		short int blockno2 = 1;
		short int blockno3;
		short int opcode2;
		if(fd<0){
			//create file
			printf("You have to create file name Wr.txt 1st\n");
		}
		while(1){
			//printf("r = %d\n", r);
			write(fd, data, r-4);
